...
| duplicate    | If a STAC entity is available under different URLs and is an exact copy, they can be exposed through this relation type. Prefer the `canonical` relation type over `duplicate` for the canonical URL. The canonical entity could for example use `duplicate` to point back to its copies. Don't provide `duplicate` and `canonical` for the same URL inside a single STAC entity.                                                         |
